...
describe('fs-file-writer', () => {
    it('store object should lock access & unlink data', async () => {
        ...
        onRelease &&
            onRelease.readHook('testRelease', ({fileName, action}) => {
                chai.expect(fileName).to.be.a('string');
            });

        const str = 'test data';
        const fName = await fileWriter.write(Buffer.from(str), {
            path,
            opts: {ext: 'txt'},
        });
        chai.expect(fName).to.be.a('string');

        const fName_02 = await fileWriter.append(Buffer.from(str + '2'), {
            fileName: fName,
        });
        ...
        const fName_03 = await fileWriter.write(Buffer.from(str), {
            fileName: fName,
        });
        ...
        const fName_04 = await fileWriter.write(Buffer.from(str + '2'), {
            fileName: fName,
        });
