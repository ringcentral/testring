...
export class FSReader extends PluggableModule implements IFSReader {
    constructor() {
        super([FSReaderPlugins.beforeResolve, FSReaderPlugins.afterResolve]);
    }

    public async find(pattern: string): Promise<IFile[]> {
        const tests = await locateFiles(pattern);
        const testsAfterPlugin = await this.callHook(
        ...
        return await this.callHook(FSReaderPlugins.afterResolve, resolvedFiles);
    }
