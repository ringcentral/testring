...
describe('Store server', () => {
    it('should init fss and test the transport lock', (done) => {
        ...
        transport.on<IFSStoreResp>(respName, ({ requestId, fileName, status, action }) => {
            chai.expect(requestId).to.be.oneOf([lockRequestID, accessRequestID, unlinkRequestID]);
            switch (action) {
        ...
        onRelease && onRelease.readHook('testRelease', ({ requestId, fileName, action }) => {
            chai.expect(requestId).to.be.oneOf([lockRequestID, accessRequestID, unlinkRequestID]);
            switch (action) {
