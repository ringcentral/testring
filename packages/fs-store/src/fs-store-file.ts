...
import { IFSStoreFile, FSStoreOptions, FSActionOptions } from '@testring/types';
...
export class FSStoreFile implements IFSStoreFile {
     ...
     * @returns
     */
    async unlock(options: FSActionOptions = { doUnlink: false, waitForUnlink: false }): Promise<boolean> {
    ...
    async unlockAll(options: FSActionOptions = { doUnlink: false, waitForUnlink: false }): Promise<boolean> {
    ...
    async transaction(cb: () => Promise<void>, unlockOpt: FSActionOptions = { doUnlink: false }) {
