...
export class BrowserProxyLocalWorker implements IBrowserProxyWorker {
    ...
    public async spawn(): Promise<void> {
        this.logger.debug(
            `Local browser proxy worker spawned with ID: ${this.workerID}`,
        );
    }
    ...
    public async kill(): Promise<void> {
        ...
        this.removeHandlers = [];
        this.browserProxy.removeHandlers.forEach((removeHandler) =>
            removeHandler(),
        );
        this.browserProxy.removeHandlers = [];
        this.logger.debug(
            `BrowserProxyLocalWorker with ID ${this.workerID} terminated.`,
        );
    }
}
