...
describe('empty options ElementPath root[\'foo*{Some text}(barName{105})\']', () => {
    ...
    describe('.__getReversedChain() call', () => {
        it('with root', () => {
            expect(childFoo.__getReversedChain()).to.be.equal('root["foo*{Some text}(barName{105})"]');
        });
        it('without root', () => {
            expect(childFoo.__getReversedChain(false)).to.be.equal('["foo*{Some text}(barName{105})"]');
        });
    });
});
